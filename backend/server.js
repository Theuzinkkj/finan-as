...
const CAT_LABELS = {
  ...
  saude:       'Saúde',       lazer:      'Lazer',       vestuario: 'Vestuário',
  educacao:    'Educação',    contas:     'Contas',       compras:   'Compras',
  outros:      'Outros',
};

function buildAnalysisPrompt(transactions, monthLabel) {
  const r2      = v => parseFloat(v).toFixed(2);
  const fmtDate = iso => { const [y, m, d] = iso.split('-'); return `${d}/${m}/${y}`; };

  const exp       = transactions.filter(t => t.type === 'despesa');
  const catTotals = {};
  exp.forEach(t => { catTotals[t.category] = (catTotals[t.category] || 0) + parseFloat(t.amount); });

  const totalExp = Object.values(catTotals).reduce((s, v) => s + v, 0);
  const totalInc = transactions.filter(t => t.type === 'receita')
    .reduce((s, t) => s + parseFloat(t.amount), 0);

  const catLines = Object.entries(catTotals)
    .sort((a, b) => b[1] - a[1])
    .map(([k, v]) => `  ${CAT_LABELS[k] || k}: R$ ${r2(v)} (${totalExp ? ((v / totalExp) * 100).toFixed(1) : 0}%)`)
    .join('\n');

  const txLines = exp.map(t => {
    const notes = t.notes ? ` | Anotação: "${t.notes}"` : '';
    return `• ${fmtDate(t.date)} | ${CAT_LABELS[t.category] || 'Outros'} | R$ ${r2(t.amount)} | ${t.description}${notes}`;
  }).join('\n');

  return `Você é um consultor financeiro pessoal. Analise os gastos de ${monthLabel}.

RESUMO:
- Receita: R$ ${r2(totalInc)} | Despesa: R$ ${r2(totalExp)} | Saldo: R$ ${r2(totalInc - totalExp)}
...
CONTEXTO — ${ctx.monthLabel}:
- Receitas: R$${Number(ctx.totalInc).toFixed(2)} | Despesas: R$${Number(ctx.totalExp).toFixed(2)} | Saldo: R$${(ctx.totalInc - ctx.totalExp).toFixed(2)}
- Por categoria: ${ctx.catSummary || 'nenhum dado'}
- Total de transações: ${ctx.txCount || 0}`;
}

// ── AI ─

app.post('/api/ai/analyze', requireAuth, aiLimiter, async (req, res, next) => {
  try {
    const { transactions, monthLabel } = req.body;
    if (!Array.isArray(transactions) || !monthLabel)
      return res.status(400).json({ message: 'Dados inválidos.' });

    const prompt = buildAnalysisPrompt(transactions, monthLabel);
    const { ok, status, data } = await proxyFetch(
...
app.post('/api/ai/chat', requireAuth, aiLimiter, async (req, res, next) => {
  try {
    ...
    const system = financialContext
      ? buildChatSystem(financialContext)
      : 'Você é um assistente financeiro pessoal. Responda em português brasileiro.';
