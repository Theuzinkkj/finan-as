...
const CloudDB = {
  ...
  async add(tx) {
    // user_id é injetado pelo backend a partir do JWT
...
// Apenas email/id de display são guardados
const Auth = {
...
const GroqAPI = {
  async analyze(transactions, monthLabel) {
    const data = await API.req('POST', '/api/ai/analyze', { transactions, monthLabel });
    if (data?.error) throw new Error(data.error.message || 'Erro na IA');
    return data.choices?.[0]?.message?.content?.trim() ?? '';
  },

  async chat(messages, financialContext) {
    const data = await API.req('POST', '/api/ai/chat', { messages, financialContext });
    if (data?.error) throw new Error(data.error.message || 'Erro na IA');
    return data.choices?.[0]?.message?.content?.trim() ?? '';
  },
};
